...
report_date <- as.Date('2024-02-01')
...
source('C:/o/OneDrive - DCCP/Traspaso/2. Cálculo de Ahorro/src/monthly report/report_month.R')

rm(list=c("Ahorro","dipres_file","summary_table"))
...
RODBC::sqlQuery(myconnDW,("if OBJECT_ID('[Estudios].[dbo].[ahorro_mensual_cm]') is not null drop table [Estudios].[dbo].[ahorro_mensual_cm]"))

RODBC::sqlSave(myconnDW,
                 dat = summary_table %>% select(datekey,convenio,total_monitoreado,monto_transado,cobertura,total_ahorro,ahorro_promedio),
                 tablename = "ahorro_mensual_cm",
                 rownames = FALSE,
                 fast = TRUE,
                 append = TRUE
  )
...
+ **Combustibles**: Estamos a la espera de la información proveniente de uno de los proveedores del convenio. Esta información se requiere para calcular el ahorro.
+ **Gas**: Debido a la no disponibilidad de la API de la Comisión Nacional de Energía (CNE) se estuvo sin información durante 3 meses. En ese periodo se utilizó la información histórica mediante un modelo ARIMA para simular el precio para cada cilindro en cada una de las regiones disponibles. Con estos datos simulados se calcula el ahorro del convenio.
